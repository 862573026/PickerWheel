package wheel.pick.sl.picker.view.base;
...
import wheel.pick.sl.picker.adapter.WheelAdapter;
import wheel.pick.sl.picker.listener.OnItemSelectedListener;
...
public class WheelView extends View {
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.wheel_view, 0, 0);
            mGravity = a.getInt(R.styleable.wheel_view_gravity, Gravity.CENTER);
            textColorOut = a.getColor(R.styleable.wheel_view_textColorOut, textColorOut);
            textColorCenter = a.getColor(R.styleable.wheel_view_textColorCenter, textColorCenter);
            dividerColor = a.getColor(R.styleable.wheel_view_dividerColor, dividerColor);
        }
    ...
    private void remeasure() {
        ...
        measuredHeight = (int) ((halfCircumference * 2) / Math.PI); //加一个条目的高度 作为
        // 求出半径
        radius = (int) (halfCircumference / Math.PI);
        // 控件宽度，这里支持weight
        measuredWidth = MeasureSpec.getSize(widthMeasureSpec);
        // 计算两条横线和控件中间点的Y位置
        firstLineY = (measuredHeight - itemHeight) / 2.0F + itemHeight; //加上标题条目的盖度
        secondLineY = (measuredHeight + itemHeight) / 2.0F +itemHeight;
...
//			canvas.drawText(label, drawRightContentStart - CENTERCONTENTOFFSET, centerY, paintCenterText);
            canvas.drawText(label, (int) ((measuredWidth - getTextWidth(paintCenterText, label)) * 0.5), maxTextHeight, paintCenterText);
        }
        counter = 0;
        while (counter < itemsVisible) {
            canvas.save();
            ...
            if (angle >= 90F || angle <= -90F) {
                ...
                float translateY = (float) (radius - Math.cos(radian) * radius - (Math.sin(radian) * maxTextHeight) / 2D) + itemHeight;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        remeasure();
        setMeasuredDimension(measuredWidth, measuredHeight + (int) itemHeight); //加一格子的高度
    }
